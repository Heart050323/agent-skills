...
Read this file only after the benefit gate in `SKILL.md` passes.

## Codex (primary)

1. Install: `brew install codex`
2. Authenticate: `codex login` with the ChatGPT account that holds the subscription, or `codex login --with-api-key` for API billing.
...
2. Authenticate: `grok login`, or set `XAI_API_KEY` for API billing.
